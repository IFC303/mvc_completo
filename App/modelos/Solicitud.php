...
class Solicitud{
    ...
    public function aceptar_socio($aceptarSocio){
        ...
        $pass=$aceptarSocio['nombre'].$aceptarSocio['id'];
         ...
         $this->db->query("INSERT INTO USUARIO (dni, nombre, apellidos, email, direccion, fecha_nacimiento, telefono, CCC, passw, talla, ha_sido,activado, id_rol,nom_pa, ape_pa, dni_pa, fecha_acep) 
...
public function solicitudes_eventos(){
    $this->db->query("SELECT id_solicitud, SOLICITUD_EVENTO.id_evento, EVENTO.nombre as nombre_evento, foto , fecha, SOLICITUD_EVENTO.nombre, apellidos, DNI, fecha_nacimiento, direccion, email, telefono
...
public function aceptar_evento($aceptar,$id){
        ...
        $this->db->query("INSERT INTO PARTICIPANTE (id_evento, nombre, apellidos, DNI, fecha_nacimiento ,direccion, email, telefono, dorsal,marca,foto) 
 ...
 public function borrar_escuela($id){
    $this->db->query("DELETE FROM solicitud_escuela WHERE id_soli_escuela=:id");
    $this->db->bind(':id', $id);
    if ($this->db->execute()) {
        return true;
    } else {
...
public function editar_escuela($editar,$id){
    $this->db->query("UPDATE solicitud_escuela set dni=:dni, nombre=:nombre, apellidos=:apellidos, cuenta=:cuenta, fecha_nacimiento=:fecha_naci, email=:email,
...
public function aceptar_escuela($aceptar,$id){
    ...
    $this->db->query("INSERT INTO socio_grupo (id_grupo,id_usuario,fecha_acep, aceptado,activo,id_categoria) 
